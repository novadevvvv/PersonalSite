...
async function loadRuntimeConfig() {
  try {
    const response = await fetch(`${BACKEND_BASE_URL}/api/runtime-config`, {
      cache: "no-store",
      headers: {
...
async function loadVideoInfo() {
  ...
  try {
    const response = await fetch(`${BACKEND_BASE_URL}/api/video-info?url=${encodeURIComponent(requestedVideoUrl)}`, {
      cache: "no-store",
      headers: {
        "Cache-Control": "no-cache",
        Pragma: "no-cache",
...
async function startTrackedDownload(options) {
  ...
  try {
    const response = await fetch(`${BACKEND_BASE_URL}/api/fake-download/start`, {
      method: "POST",
      headers: {
...
async function pollDownloadProgress(jobId, progressTemplate) {
  try {
    const response = await fetch(`${BACKEND_BASE_URL}/api/fake-download/status/${encodeURIComponent(jobId)}`, {
      cache: "no-store",
      headers: {
        "Cache-Control": "no-cache",
        Pragma: "no-cache",
...
function triggerTrackedDownload(jobId) {
  ...
  link.href = `${BACKEND_BASE_URL}/api/fake-download/file/${encodeURIComponent(jobId)}`;
  link.rel = "noopener";
